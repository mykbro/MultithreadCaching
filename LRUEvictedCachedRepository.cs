...
namespace MultithreadCaching
{
	...
	{
		private readonly LinkedList<TValue> _keyListByMostRecent = new LinkedList<TValue>();
		private readonly Dictionary<TValue, LRUCacheEntry<TValue, TResult>> _results;		//we now use a Dictionary of Value -> { Task<Result>, LinkedListNode<Value> }
		...
		public TResult GetResult(TValue v)
		{
			...
			lock (_globaLock)
			{
				...
				else 
				{
					...
					// ...and we need to update the LRU list !
					...
					_keyListByMostRecent.AddFirst(node); // we place it at the beginning					
				}
			}
			...
			return resultTask!.Result;    //we block until a Result arrives
		}	
